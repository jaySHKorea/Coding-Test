...
'''
무지는 단순한 문자 코드 순이 아닌,
파일명에 포함된 숫자를 반영한 정렬 기능을
저장소 관리 프로그램에 구현하기로 했다.
...
앞쪽에 0이 올 수 있다. 0부터 99999 사이의 숫자로,
...
영문 대소문자, 숫자, 공백(" ), 마침표(.), 빼기 부호(-")만으로 이루어져 있다.
파일명은 영문자로 시작하며, 숫자를 하나 이상 포함하고 있다.
중복된 파일명은 없으나, 대소문자나 숫자 앞부분의 0 차이가 있는 경우는
함께 주어질 수 있다.
...
def solution(files):
    ...
    for f_name in files:
        ...
        # Head / Number / Tail
        for c in f_name:
            if (c.isdigit() and flag == False): # 첫 숫자 일때
                tmp.append(f_name[start:idx])
                flag = True
                start = idx
            elif ( (not c.isdigit()) and flag == True):
                tmp.append(f_name[start:idx])
                #tmp.append(f_name[idx:])
                break
            idx += 1
        ...
        for c in tmp[1]:
            if ( c != '0'):
                ...
                break
            idx += 1
        answer.append([f_name,tmp[0],tmp[1]])

    answer = sorted(answer,key=lambda x: (x[1].lower(),x[2]))
